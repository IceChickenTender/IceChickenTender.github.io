...
멀티-헤드는 "Attention is all you need" 에서 설명하길 병렬처리를 하기 위한 방법이라고 소개가 됩니다. 이후에 자세하게 설명을 할 거지만 Transformer 의 self-attention 기법에는 Q, K, V 세 가지 정보를 가지는 weight 가 있으며 이 weight 들은 모델에 지정한 차원x차원이 됩니다. 그리고 이 weight 들이 학습이 되면서 우리가 원하는 출력을 얻을 수 있게 됩니다. 그리고 멀티-헤드는 이 weight 들을 연산할 때 우리가 지정한 head 의 수로 차원을 나누어주게 되고, 나눈 차원들이 병렬적으로 동시에 연산이 수행이 되므로 loop 를 이용해 하나 하나 순차적으로 연산할 때에 비해서 속도가 굉장히 빨라지게 됩니다.
